namespace TPSBR
{
	...
	public class Character : MonoBehaviour
	{
		...
		[SerializeField]
		private float _sprintFOVBonus = 6f;
		[SerializeField]
		...
		public void OnSpawned(Agent agent)
		{
			...
			_cameraDistance = GetCameraTransform(ECameraState.Default, false).LocalPosition.magnitude * Global.RuntimeSettings.CameraDistance;
		...
		public void OnAgentRender()
		{
			...
			float targetFOV = _characterController.Data.Aim == true ? aimFOV : _defaultFOV;
			if (_characterController.Data.Aim == false && _characterController.Data.Sprint == true)
			{
				targetFOV += _sprintFOVBonus;
			}
			...
			if (_previousCameraState != _currentCameraState || _previousLeftSide != _currentLeftSide)
			{
				...
				float cameraDistMultiplier = Global.RuntimeSettings.CameraDistance;
				float maxCameraDistance = currentCameraTransform.LocalPosition.magnitude * cameraDistMultiplier;
				...
				// Calculate player pivot (camera anchor without offset)
				Vector3 prevPivot = previousCameraTransform.Position - previousCameraTransform.LocalPosition;
				Vector3 currPivot = currentCameraTransform.Position - currentCameraTransform.LocalPosition;
				Vector3 playerPivot = Vector3.Lerp(prevPivot, currPivot, progress);

				// Raycast from player pivot towards camera
				if (_agent.Runner.GetPhysicsScene().Raycast(playerPivot, raycastDirection, out RaycastHit hitInfo, maxCameraDistance + 0.25f, -5, QueryTriggerInteraction.Ignore) == true)
				{
				...
				_camera.transform.position = playerPivot + raycastDirection * _cameraDistance;
			...
			else
			{
				...
				float cameraDistMultiplier = Global.RuntimeSettings.CameraDistance;
				float maxCameraDistance = cameraTransform.LocalPosition.magnitude * cameraDistMultiplier;
				_cameraDistance = Mathf.Clamp(_cameraDistance + maxCameraDistance * 8.0f * Time.deltaTime, 0.0f, maxCameraDistance);

				Vector3 raycastDirection = Vector3.Normalize(cameraTransform.LocalPosition);

				// Calculate player pivot (camera anchor without offset)
				Vector3 playerPivot = cameraTransform.Position - cameraTransform.LocalPosition;
		...
		private void SetCameraState(ECameraState state, bool leftSide)
		{
			...
			_cameraDistance = Mathf.Max(_cameraDistance, GetCameraTransform(_currentCameraState, _currentLeftSide).LocalPosition.magnitude * Global.RuntimeSettings.CameraDistance);
		}
