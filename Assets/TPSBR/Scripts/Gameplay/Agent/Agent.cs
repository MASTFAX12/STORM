...
namespace TPSBR
{
	...
	public sealed class Agent : ContextBehaviour, ISortedUpdate
	{
		...
		[SerializeField]
		private bool _allowFireWhileSprinting = false;
		...
		public override void Spawned()
		{
			...
			void CheckInitialGround()
			{
				...
				// Brief delay check for grounding
				const float checkDist = 5f;
				int groundMask = 1 << 0; // Default layer

				if (!Physics.Raycast(transform.position + Vector3.up, Vector3.down, out _, checkDist + 1f, groundMask))
				{
					Debug.LogWarning($"[AgentSafety] Agent {name} spawned with no ground detected at {transform.position}. Check spawn point and MapPlayArea.");
				}
			}
		...
		private void ProcessRenderInput()
		{
			...
			if (_health.IsAlive == true)
			{
				...
				input.Sprint            = accumulatedInput.Sprint;
			}
		...
		{
			if (_allowFireWhileSprinting == false && _character.CharacterController.FixedData.Sprint == true)
				return;
		...
		private bool CanSprint(GameplayInput input)
		{
			if (input.Sprint == false || input.MoveDirection.IsZero() == true)
				return false;
			if (input.Aim == true || _jetpack.IsActive == true || _health.IsAlive == false)
				return false;
			if (_allowFireWhileSprinting == false && input.Attack == true)
				return false;
