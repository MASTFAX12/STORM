...
namespace TPSBR
{
	public sealed class BattleRoyaleGameplayMode : GameplayMode
	{
		...
		public bool RequestAirplaneJump(PlayerRef playerRef, Vector3 direction)
		{
			...
			var spawnPosition = ValidateSpawnPosition(_airplane.AgentPosition.position);
			var agent = SpawnAgent(playerRef, spawnPosition, Quaternion.LookRotation(direction.OnlyXZ()));
		...
		protected override void TrySpawnAgent(Player player)
		{
			...
			{
				// Fallback to base direct spawn if airplane is not active (late joiners or custom cases)
				base.TrySpawnAgent(player);
			}
		}
		...
		{
			var playArea = Runner.SimulationUnityScene.GetComponent<MapPlayArea>();
			if (playArea == null)
			{
				// Fallback to old shrinking area logic if MapPlayArea is missing
				...
				var lookDir = Vector3.Cross((_shrinkingArea.Center - position).OnlyXZ(), Vector3.up);
				lookDir = Random.value > 0.5f ? -lookDir : lookDir;

				_airplane = Runner.Spawn(_airplanePrefab, position, Quaternion.LookRotation(lookDir));
				AirplaneActive = true;
				return;
			}
			...
			// Define a random path across the map
			float angle = Random.Range(0f, 360f) * Mathf.Deg2Rad;
			Vector2 dir = new Vector2(Mathf.Cos(angle), Mathf.Sin(angle));
			
			float mapSize = Mathf.Max(bounds.size.x, bounds.size.z);
			float startDist = mapSize * 0.7f; // Start outside
			
			Vector3 center = bounds.center;
			Vector3 startPos = center + new Vector3(dir.x, 0, dir.y) * -startDist;
			Vector3 endPos = center + new Vector3(dir.x, 0, dir.y) * startDist;
			
			// Find highest terrain point under path to set safe flight height
			float maxHeight = playArea.GetTerrainHeight(center);
			maxHeight = Mathf.Max(maxHeight, playArea.GetTerrainHeight(startPos));
			maxHeight = Mathf.Max(maxHeight, playArea.GetTerrainHeight(endPos));
			
			startPos.y = maxHeight + _airplaneHeight;
			endPos.y = startPos.y;

			_airplane = Runner.Spawn(_airplanePrefab, startPos, Quaternion.LookRotation(endPos - startPos));
			_airplane.NextTargetPosition = endPos; // Airplane.cs uses this for navigation
