namespace TPSBR
{
	...
	public sealed class NetworkGame : ContextBehaviour, IPlayerJoined, IPlayerLeft
	{
		...
		[SerializeField]
		private float _runtimeItemBoxMinSpacing = 10.0f;
		...
		private readonly List<Vector3>        _spawnedItemBoxPositions = new List<Vector3>(256);
		...
		private void GenerateLevel(int seed)
		{
			...
			_spawnedItemBoxPositions.Clear();

			for (int i = 0; i < _levelGenerator.ItemBoxesToSpawn.Count; i++)
			{
				var spawnData = _levelGenerator.ItemBoxesToSpawn[i];
				if (GroundSnapUtility.TryGetNonOverlappingGroundPosition(spawnData.Position, _spawnedItemBoxPositions, _runtimeItemBoxMinSpacing, out Vector3 spawnPosition) == false)
					continue;

				Runner.Spawn(spawnData.Prefab, spawnPosition, spawnData.Rotation);
				_spawnedItemBoxPositions.Add(spawnPosition);
			}
		}
