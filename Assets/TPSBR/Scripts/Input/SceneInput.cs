...
namespace TPSBR
{
	...
	public class SceneInput : SceneService
	{
		...
		protected override void OnTick()
		{
			...
			{
				bool backPressed = Keyboard.current != null && Keyboard.current.escapeKey.wasPressedThisFrame == true;

				// Gamepad Start/Menu button (≡) = same as Escape
				if (Gamepad.current != null && Gamepad.current.startButton.wasPressedThisFrame == true)
				{
					backPressed = true;
				}
